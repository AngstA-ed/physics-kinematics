...
All 10 units are authored. **Kinematics** is the original interactive pilot
(rich HTML student pages + DOCX). The other nine units are **DOCX-only**:
each lesson ships a Teacher Guide, Student Worksheet, Student Notes, and Answer
Key as `.docx`, with embedded charts/graphs/diagrams — no HTML (see
"DOCX-only units" below).

- 00 Math in Science — 4 lessons (DOCX-only)
- **01 Kinematics — pilot** — 9 lessons (interactive HTML + DOCX)
- 02 Forces — 6 lessons (DOCX-only)
- 03 Momentum / Impulse — 4 lessons (DOCX-only)
- 04 Energy — 6 lessons (DOCX-only)
- 05 Thermodynamics — 3 lessons (DOCX-only)
- 06 Electrostatics — 6 lessons (DOCX-only)
- 07 Current Electricity — 5 lessons (DOCX-only)
- 08 Waves — 10 lessons (DOCX-only)
- 09 Modern Physics — 5 lessons (DOCX-only)

### DOCX-only units (00, 02–09)

Authored after the Kinematics pilot, these units skip the interactive HTML
student page. Each lesson folder holds four Markdown sources — `Teacher_Guide.md`,
`Student_Worksheet.md`, `Student_Notes.md`, `Answer_Key.md` — built to `.docx`
only. Figures live in each lesson's `figures/` folder (generated by
`tools/figures.py` and the `tools/_gen_*_figures.py` scripts) and embed into the
DOCX via pandoc's `--resource-path`. Scaffold new ones with
`python tools/scaffold_lesson.py <UNIT> NN "Title" --docx-only --strategies "..."`.
